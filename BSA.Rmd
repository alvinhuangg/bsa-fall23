...
# normalizing 'pts' column, 'champ' column, 'all_star' columnn
df$pts <- (df$pts-min(df$pts))/(max(df$pts)-min(df$pts))*100
df$champ <- (df$champ-min(df$champ))/(max(df$champ)-min(df$champ))*100
df$all_star <- (df$all_star-min(df$all_star))/(max(df$all_star)-min(df$all_star))*100

# obtaining the percent values instead of the proportion value
df$ast_pct <- df$ast_pct*100
df$reb_pct <- df$reb_pct*100
...
# subsetting players with scores > 25 from 'undrafted' to obtain those who had scores over the floor of the y-intercept value obtained earlier as well as for visualization purposes
undrafted_score_greater_than_25 <- undrafted[which(undrafted$score > 25),]
...
# creating barplot of undrafted players who have a score greater than 25 with their respective scores
par(mar = c(4, 8, 4, 2))
barplot(undrafted_score_greater_than_25$score, names.arg = undrafted_score_greater_than_25$player_name, col = "blue", main = "Undrafted Player Scores Over 25", xlab = "Scores", ylab = "", horiz=T, las=1)
title(ylab="Players", line=7.1, cex.lab=0.8)
